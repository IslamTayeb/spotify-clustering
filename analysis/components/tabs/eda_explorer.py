...
def render_eda_explorer(df: pd.DataFrame):
    ...
    with st.expander(
        "🧮 Interpretable Embedding Features", expanded=True
    ):
        st.subheader("Feature Vector Analysis - Clustering Features")
        st.caption(
            "These normalized features are the EXACT values used for clustering"
        )

        # Look for emb_* columns (the embedding values used for clustering)
        ...
        if not embedding_feature_cols:
            ...
            st.caption(
                f"Analyzing all {len(embedding_feature_cols)} embedding dimensions used for clustering"
            )
            ...
            st.markdown("### 📈 Feature Statistics")
            st.caption(
                "Statistics for features that are part of the clustering embedding"
            )
            ...
            stats_df = pd.DataFrame(
                {
                    ...
                    "Range": [
                        df[col].max() - df[col].min() for col in embedding_feature_cols
                    ],
                }
            )
            ...
            export_df = df[
                ["track_name", "artist", "cluster"] + embedding_feature_cols
            ].copy()
    ...
    with st.expander(
        "📊 Raw Dataframe Features (not used in clustering)", expanded=False
    ):
        ...
        if not raw_feature_cols:
            st.info(
                "No additional raw features found (all numeric columns are in the embedding)"
            )
        else:
            st.warning(
    ...
    with st.expander("🎸 Genre Analysis", expanded=False):
        if "top_genre" in df.columns:
            # Parent genre distribution (grouped)
            st.subheader("Genre Family Distribution")
            st.caption(
                "Genres grouped by parent category (e.g., all Hip Hop subgenres combined)"
            )

            def extract_parent_genre(genre_str):
                """Extract parent genre from 'Category---Subgenre' format."""
            ...
            with col1:
                fig_pie = px.pie(
                    ...
                    hole=0.3,
                    color_discrete_sequence=get_pie_colors(
                        parent_counts_grouped.index, CLUSTER_COLORS
                    ),
                )
    ...
    with st.expander("🌍 Language Analysis", expanded=False):
        if "language" in df.columns or "lyric_language" in df.columns:
            ...
            fig = px.pie(
                ...
                hole=0.3,
                color_discrete_sequence=get_pie_colors(
                    lang_counts_grouped.index, CLUSTER_COLORS
                ),
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            if top_langs:
                ...
                for i, lang in enumerate(top_langs):
                    with cols[i]:
                        ...
                        lang_songs = df[df[lang_col] == lang].sample(
                            min(5, len(df[df[lang_col] == lang]))
                        )
    ...
    with st.expander("⏰ Temporal Analysis", expanded=False):
        ...
        if not has_temporal:
            ...
            df_temp["release_date"] = pd.to_datetime(
            ...
            if valid_temporal.sum() == 0:
                ...
                # Filter out data before June 4, 2024 for temporal analysis
                ...
                # Count songs before cutoff (the "starting point")
                ...
                monthly_additions = df_temp.groupby(
                    df_temp["added_at"].dt.to_period("M")
                ).size()
                ...
                if df_temp["release_year"].notna().sum() > 0:
                    ...
                    with col2:
                        ...
                        # Format names: "1990s" for decades, keep "Other (N)" as-is
                        decade_names = [
                            f"{int(d)}s" if isinstance(d, (int, float)) else d
                            for d in decade_counts_grouped.index
                        ]
                        fig = px.pie(
                    ...
                    try:
                        ...
                        for period in ["Period 1", "Period 2", "Period 3", "Period 4"]:
                            if period in df_sorted_cluster["time_period"].values:
                                period_df = df_sorted_cluster[
                                    df_sorted_cluster["time_period"] == period
                                ]
                                min_d = period_df["added_at"].min().strftime("%Y-%m-%d")
                                max_d = period_df["added_at"].max().strftime("%Y-%m-%d")
                                st.write(
                                    f"- {period}: {min_d} → {max_d} ({len(period_df)} songs)"
                                )
                ...
                with col1:
                    ...
                    if df_temp["release_year"].notna().sum() > 0:
                        ...
                        if len(df_valid_years) > 0:
                            oldest = df_valid_years.loc[
                                df_valid_years["release_year"].idxmin()
                            ]
                            st.text(f"Oldest: {oldest['track_name']}")
                            st.caption(
                                f"Released: {int(oldest['release_year'])} | by {oldest['artist']}"
                            )

                            newest = df_valid_years.loc[
                                df_valid_years["release_year"].idxmax()
                            ]
                            st.text(f"Newest: {newest['track_name']}")
                            st.caption(
                                f"Released: {int(newest['release_year'])} | by {newest['artist']}"
                            )
                    else:
                        st.info("Release year data not available")
                ...
                with col2:
                    ...
                    st.caption(
                        f"on {first['added_at'].strftime('%Y-%m-%d')} | by {first['artist']}"
                    )

                    last = df_temp.loc[df_temp["added_at"].idxmax()]
                    st.text(f"Last Added: {last['track_name']}")
                    st.caption(
                        f"on {last['added_at'].strftime('%Y-%m-%d')} | by {last['artist']}"
                    )
                ...
                if (
                    "age_at_add_years" in df_temp.columns
                    and df_temp["age_at_add_years"].notna().sum() > 0
                ):
                    valid_ages_mask = df_temp["age_at_add_years"].between(0, 100)
                    df_valid_ages = df_temp[valid_ages_mask]

                    if len(df_valid_ages) > 0:
                        vintage = df_valid_ages.loc[
                            df_valid_ages["age_at_add_years"].idxmax()
                        ]
                        st.text(
                            f"Most Vintage: {vintage['track_name']} ({vintage['age_at_add_years']:.1f} yrs old)"
                        )

                        brand_new = df_valid_ages.loc[
                            df_valid_ages["age_at_add_years"].idxmin()
                        ]
                        st.text(
                            f"Newest Release: {brand_new['track_name']} ({brand_new['age_at_add_years']:.1f} yrs old)"
                        )
                else:
                    st.info("Age data not available")
                ...
                show_cluster_trendlines = st.checkbox(
                    "Show trend lines",
                    value=True,
                    key="cluster_trends_trendlines",
                )
                ...
                if len(df_temp) >= 30 and "cluster" in df_temp.columns:
                    ...
                    cluster_dummies = pd.get_dummies(
                        df_sorted_cluster_trend["cluster"], prefix="cluster"
                    )

                    # Apply rolling window (same as mood trends)
                    rolling_clusters = (
                        cluster_dummies.rolling(window=30, min_periods=10).mean() * 100
                    )
                    rolling_clusters["added_at"] = df_sorted_cluster_trend[
                        "added_at"
                    ].values
                    ...
                    cluster_cols = [
                        col
                        for col in rolling_clusters.columns
                    ...
                    rolling_melted["Cluster"] = rolling_melted["Cluster"].str.replace(
                        "cluster_", "Cluster "
                    )
                    ...
                    if show_cluster_trendlines:
                        ...
                        for i, cluster in enumerate(rolling_melted["Cluster"].unique()):
                            cluster_data = rolling_melted[
                                rolling_melted["Cluster"] == cluster
                            ].dropna()
                            if len(cluster_data) > 1:
                                x_numeric = (
                                    cluster_data["added_at"]
                                    - cluster_data["added_at"].min()
                                ).dt.total_seconds()
                ...
                show_mood_trendlines = st.checkbox(
                    "Show trend lines",
                    value=True,
                    key="mood_trends_trendlines",
                )
                ...
                if available_moods and len(df_temp) >= 30:
                    ...
                    rolling_moods = (
                        df_sorted_mood[available_moods]
                    ...
                    if show_mood_trendlines:
                        st.caption(
                            "Dashed lines show overall trend direction for each mood"
                        )
                elif len(df_temp) < 30:
                ...
                show_genre_trendlines = st.checkbox(
                    "Show trend lines",
                    value=True,
                    key="genre_family_trendlines",
                )
                ...
                if "top_genre" in df_temp.columns and len(df_temp) > 0:
                    ...
                    df_temp["genre_family"] = df_temp["top_genre"].apply(
                        get_genre_family
                    )
                    ...
                    top_families = (
                        df_temp["genre_family"].value_counts().head(6).index.tolist()
                    )
                    ...
                    for quarter in quarters:
                        ...
                        for family in top_families:
                            added_this_quarter = (
                                quarter_df["genre_family"] == family
                            ).sum()
                    ...
                    if timeline_data:
                        ...
                        for family in top_families:
                            ...
                            timeline_df.loc[family_mask, "Delta"] = (
                                timeline_df.loc[family_mask, "Quarter %"]
                                .diff()
                                .fillna(0)
                            )
                        ...
                        if genre_view == "Quarterly Proportion":
                            ...
                            if show_genre_trendlines:
                                colors = GENRE_FAMILY_COLORS
                                for i, family in enumerate(top_families):
                                    family_data = timeline_df[
                                        timeline_df["Genre Family"] == family
                                    ]
                                    if len(family_data) > 1:
                                        x_numeric = np.arange(len(family_data))
                                        z = np.polyfit(
                                            x_numeric, family_data["Quarter %"], 1
                                        )
                                        p = np.poly1d(z)
                                        fig.add_trace(
                                            go.Scatter(
                            ...
                            if show_genre_trendlines:
                                ...
                                for i, family in enumerate(top_families):
                                    ...
                                    if len(family_data) > 1:
                                        x_numeric = np.arange(len(family_data))
                                        z = np.polyfit(
                                            x_numeric, family_data["Cumulative"], 1
                                        )
                                        p = np.poly1d(z)
                                        fig.add_trace(
                                            go.Scatter(
                        ...
                        for i, family in enumerate(top_families[:4]):
                            ...
                            family_pct = (
                                family_total / total_songs * 100
                                if total_songs > 0
                                else 0
                            )
                            family_deltas = timeline_df[
                                timeline_df["Genre Family"] == family
                            ]["Delta"]
                ...
                if len(df_temp) > 0 and "cluster" in df_temp.columns:
                    ...
                    try:
                        cluster_month_matrix = (
                            df_temp.groupby(["month", "cluster"])
                            .size()
                            .unstack(fill_value=0)
                        )

                        if (
                            len(cluster_month_matrix) > 1
                            and len(cluster_month_matrix.columns) > 1
                        ):
                            fig = px.imshow(
                                cluster_month_matrix.T,
                                labels=dict(
                                    x="Month", y="Cluster", color="Songs Added"
                                ),
                            ...
                            fig.update_layout(
                                height=400 + len(cluster_month_matrix.columns) * 20
                            )
                            st.plotly_chart(fig, use_container_width=True)
                        else:
                    ...
                    if cluster_id == parent_cluster:
                        ...
                        fig.add_trace(
                            go.Scatter3d(
                                ...
                                marker=dict(
                                    size=3, color="rgba(128,128,128,0.3)", opacity=0.3
                                ),
                                text=hover_texts,
                                hovertemplate="%{text}<extra></extra>",
                            )
                ...
                for i, cluster_id in enumerate(unique_clusters):
                    ...
                    fig.add_trace(
                        go.Scatter3d(
                            ...
                            name=f"Cluster {cluster_id} ({len(cluster_df)})",
                            marker=dict(
                                size=4, color=colors[i % len(colors)], opacity=0.8
                            ),
                            text=hover_texts,
                            hovertemplate="%{text}<extra></extra>",
                        )
