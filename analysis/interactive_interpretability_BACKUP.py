...
def render_eda_explorer(df: pd.DataFrame):
    ...
    with st.expander("🌍 Language Analysis", expanded=False):
        if "language" in df.columns or "lyric_language" in df.columns:
            # Use whichever column is available
            lang_col = "language" if "language" in df.columns else "lyric_language"

            st.subheader("Language Distribution")
            st.caption(
                "Language detection is performed on lyric text using langdetect. "
                "Instrumental tracks or songs without lyrics are marked as 'none' or 'unknown'."
            )
            ...
            # Sample songs by language
            st.markdown("---")
            st.subheader("Sample Songs by Language")

            # Show top 3 languages (excluding 'none' and 'unknown')
            ...
            # Language Ladder (if available)
            st.markdown("---")
            st.subheader("Language Encoding in Features")
            st.caption(
                "In interpretable mode, languages are encoded using an ordinal scale for clustering. "
                "This helps the algorithm understand language relationships while keeping the feature space compact."
            )

            st.info(
                "**Language Ladder** (1 dimension):\n\n"
                "Languages are encoded on a 0-1 scale to preserve ordinality in clustering:\n"
                "- English: 1.0\n"
                "- Spanish: 0.86\n"
                "- French: 0.71\n"
                "- Arabic: 0.57\n"
                ...
                "This encoding helps clustering algorithms understand that 'English' and 'Spanish' are more similar "
                "than 'English' and 'Japanese', while keeping the feature to just 1 dimension instead of 8 one-hot columns."
            )
            ...
            if "instrumentalness" in df.columns:
                ...
                st.caption("Understanding the relationship between detected language and vocal content")
                ...
                fig = px.box(
                    ...
                    labels={
                        lang_col: "Language",
                        "instrumentalness": "Instrumentalness (0=vocal, 1=instrumental)"
                    },
                )
                st.plotly_chart(fig, use_container_width=True)
                ...
                # Stats
                st.write("**Analysis:**")
                none_instr = df[df[lang_col] == 'none']['instrumentalness'].mean() if 'none' in df[lang_col].values else 0
                if none_instr > 0.5:
                    st.write(f"✓ Songs marked as 'none' have high instrumentalness ({none_instr:.2f}), confirming they're instrumental tracks.")
                else:
                    st.write(f"⚠️ Some songs marked as 'none' have low instrumentalness ({none_instr:.2f}), suggesting lyrics may be missing.")
        else:
            st.warning("Language information not available in this dataset")
