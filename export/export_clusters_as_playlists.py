...
def get_cluster_description(cluster_stats, cluster_id, mode="audio"):
    """Generate cluster description from statistics."""
    ...
    if mode == "audio" or mode == "combined":
        ...
        top_genre = top_3_genres[0][0] if top_3_genres else "Mixed"
...
def preview_cluster_playlists(clusters, cluster_stats, mode="audio", prefix=""):
    """Preview playlists for each cluster."""
    print(f"\n{'=' * 70}")
...
def export_clusters_to_playlists(
    ...
    """Create and populate Spotify playlists for each cluster."""
...
def main():
    ...
    if not data_path.exists():
        ...
        print("Please run the analysis pipeline first: python analysis/run_analysis.py")
        return
